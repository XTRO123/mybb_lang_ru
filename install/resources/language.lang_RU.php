...
$l['req_step_error_configdefaultfile'] = 'Файл конфигрурации (inc/config.default.php) не может быть переименован. Please manually rename the <u>config.default.php</u> file to <u>config.php</u> to allow it to be written to or contact <a href="http://www.mybb.com/support" target="_blank">MyBB Support.</a>';
...
$l['db_step_config_db'] = '<p>Сейчас настало время для настройки базы данных, которую будет использовать MyBB, а также указать ваши данные аутентификации для базы данных. Если у вас нет этой информации, как правило, её можно получить у вашего веб-хостинга.</p>';
...
<h3>Ошибка</h3>
<p>Здесь, кажется, одна или несколько ошибок с информацией о конфигурации базы данных, которые вы предоставили:</p>
{1}
...
$l['db_step_error_utf8mb4_error'] = '\'4-Byte UTF-8 Unicode\' требует MySQL 5.5.3 или выше.Пожалуцста, выберите кодировку, совместимую с вашей MySQL версией.';
...
<h3>Ошибка</h3>
<p>Здесь, кажется, одна или несколько ошибок в конфигурации доски,в данных, которые вы предоставили:</p>
{1}
<p>После исправления продолжите установку.</p>
</div>';
...
$l['admin_step_setupsettings'] = '<p>Настройка основных параметров доски...</p>';
...
$l['admin_step_createadmin'] ='<p>Вам нужно создать вашу начальную учетную запись администратора , чтобы войти в систему и управлять вашей копии MyBB. Пожалуйста, заполните необходимые поля ниже, чтобы создать эту учетную запись.</p>';
...
<h3>Ошибка</h3>
<p>Здесь, кажется, одна или несколько ошибок с конфигурацией доски, которую вы предоставили:</p>
{1}
<p>После исправления продолжите установку.</p>
</div>';
...
$l['admin_step_error_noemail'] = 'Вы не ввели свой адрес электронной почты для учетной записи администратора.';
$l['admin_step_nomatch'] = ' Повтор пароля не совпадает с паролем, с первого поля. Пожалуйста, исправьте перед продолжением';
...
$l['upgrade_settings_sync_success'] = "<p>Параметры доски были синхронизированы с последним в MyBB.</p><p>{1} новых настроек, вставлены вместе с {2} новыми параметрами групп.</p><p>Для завершения обновления, пожалуйста, нажмите кнопку Далее, чтобы продолжить.</p>";
...
$l['upgrade_locked'] = "<p>Этот инсталятор был заблокирован. Чтобы разблокировать программу установки, пожалуйста, удалите файл \'lock\' в каталоге.</p><p>Теперь вы можете перейти на сайт <a href=\"../index.php\">MyBB</a> или в <a href=\"../{1}/index.php\">Панель управления Администратора</a>.</p>";
$l['upgrade_removedir'] = 'Пожалуйста, удалите эту директорию перед осмотром вашего обновления MyBB.';
$l['upgrade_congrats'] = "<p>Поздравляем, ваша копия MyBB была успешно обновлена до {1}.</p>{2}<p><strong>Что дальше?</strong></p><ul><li>Пожалуйста, используйте инструмент 'Найти обновленные шаблоны'  в админпанели, чтобы найти настроенные шаблоны во время этого процесса обновления. Отредактируйте содержимое изменённых или восстановите их оригиналы.</li><li>Убедитесь, что ваша доска по-прежнему полностью функциональна.</li></ul>";
$l['upgrade_template_reversion'] = "Предупреждение резервирования шаблона";
$l['upgrade_template_reversion_success'] = "<p>Все необходимые изменения базы данных были успешно сделаны для обновления доски.</p><p>Это обновление требует, чтобы все шаблоны,которые содержатся в пакете, будут перезаписаны,  поэтому, пожалуйста, сделайте резервную копию всех пользовательских шаблонов, которые вы сделали, прежде чем нажимать дальше.";
...
/* Error messages */
$l['development_preview'] = "<div class=\"error\"><h2 class=\"fail\">Внимание</h2><p>Эта версия MyBB предназначена для разработки и должна использоваться только для целей тестирования.</p><p>Нет официальной поддержки, кроме как для плагинов и  темы, не будет предоставляться для этой версии. Продолжая эту установку / обновление, вы делаете это на свой страх и риск.</p></div>";
